...
class CMEModule:
    ...
    def on_login(self, context, connection):
        ...
        logging.debug(f'StringBinding {self.__stringbinding}')
        ...
        for endpoint in list(endpoints.keys()):
            if "MS-RPRN" in endpoints[endpoint]['Protocol']:
                logging.debug(f"Protocol: {endpoints[endpoint]['Protocol']} ")
                logging.debug(f"Provider: {endpoints[endpoint]['EXE']} ")
                logging.debug(f"UUID    : {endpoint} {endpoints[endpoint]['annotation']}")
